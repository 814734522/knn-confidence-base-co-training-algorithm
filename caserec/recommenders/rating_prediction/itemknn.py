...
class ItemKNN(BaseKNN):
    ...
    def getknn(self, user, item):
        ...
        knn = []
        i_id = self.item_to_item_id[item]
        si_matrix = self.compute_similarity(transpose=True, nomalize=False)    # 获得相似矩阵
        neighbor = sorted(range(len(si_matrix[i_id])), key=lambda x: -si_matrix[i_id][x], reverse=True)  # 返回降序的用户邻近id

        for i in neighbor:
            item1 = self.item_id_to_item[i]                                     # id 转为用户
            if user in self.train_set['users_viewed_item'][item1]:
                knn.append(self.train_set['feedback'][user][item1])
        if len(knn) == 0:
            return False                                                        # 没有邻近集
