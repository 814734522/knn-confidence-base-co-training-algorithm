...
class BprMF(BaseItemRecommendation):
    ...
    def init_model(self):
        ...
        # Upgrade unobserved items with test set samples    改良测试集样本中的未评分对象
        if self.items_test:
            for u, user in enumerate(self.users):
                self.train_set['items_unobserved'][user] = list(set(self.items) -
                                                                set(self.train_set['items_seen_by_user'][user]))

        # Initialize factors                                初始化特征
        self.create_factors()

        # Define number of interactions in each epoch       定义每个时期的交互操作数
    ...
    def create_factors(self):                       # 生成带有正态分布的均值方差的特征向量
        """
    ...
    def sample_pair(self, user):                    # 随机选取部分用户已知和未知的对象
        """
    ...
    def predict_score(self, user, item):                # 预测评分
        """
    ...
    def update_factors(self, u, i, j):                          # 根据随机梯度下降修改潜在因子数码
        """
        ...
        # Compute Difference                计算量预测评分间的差别度  logic回归函数
        x_uij = self.bias[i] - self.bias[j] + (self.predict_score(u, i) - self.predict_score(u, j))
        eps = 1 / (1 + np.exp(x_uij))

        self.bias[i] += self.learn_rate * (eps - self.reg_bias * self.bias[i])          # 更新对象i的误差
        self.bias[j] += self.learn_rate * (-eps - self.reg_bias * self.bias[j])         # 更新对象j的误差
