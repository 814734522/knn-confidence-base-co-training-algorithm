...
class ReadFile(object):
    def __init__(self, input_file, sep=',', header=None, names=None, as_binary=False, binary_col=2):    # \t改为','
        """
        ...
        with open(self.input_file) as infile:
            for line in infile:
                if line.strip():
                    ...
                    items_seen_by_user.setdefault(user, set()).add(item)                # 用户看过的对象集
                    users_viewed_item.setdefault(item, set()).add(user)                 # 看过该对象的用户集
    ...
    def read_metadata_or_similarity(self):
        ...
        with open(self.input_file) as infile:
            for line in infile:
                if line.strip():
                    ...
                    if len(inline) == 2:
                        attr1, attr2 = int(inline[0]), int(inline[1])
                        dict_values.setdefault(attr1, {}).update({attr2: 1.0})
                        list_col_1.add(attr1)
                        list_col_2.add(attr2)
                        number_interactions += 1
                    else:    # 三列为相似度文件
                        attr1, attr2, value = int(inline[0]), int(inline[1]), float(inline[2])
        ...
        # print(dict_file['number_interactions'])
        return dict_file
    ...
    def write(self):
        ...
        with open(self.output_file, self.mode) as infile:
            for triple in self.data:
                if self.as_binary:
                    infile.write('%d%s%d%s%f\n' % (triple[0], self.sep1, triple[1], self.sep1, 1.0))
                else:
                    infile.write('%d%s%d%s%f\n' % (triple[0], self.sep1, triple[1], self.sep1, triple[2]))
        ...
        with open(self.output_file, self.mode) as infile:
            for user in self.data:
                for pair in self.data[user]:
                    infile.write('%d%s%d%s%f\n' % (user, self.sep1, pair[0], self.sep1, pair[1]))
