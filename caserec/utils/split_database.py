...
class SplitDatabase(ReadFile):
    ...
    def write_files(self, trained_model):
        ...
        for train_index, test_index in trained_model:
            if self.dir_folds is not None:
                ...
                WriteFile(train_file, sep=',', mode=self.write_mode             # self.write_sep 改为','
                          ).write_with_pandas(df_train.sort_values(by=[0]))
                WriteFile(test_file, sep=',', mode=self.write_mode
                          ).write_with_pandas(df_test.sort_values(by=[0]))
    ...
    def shuffle_split(self, test_size=0.2, random_state=None):
        """
        Shuffle Split
